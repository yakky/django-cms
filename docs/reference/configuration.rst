...
``text_plugins``
    List of plugins available in the TextPlugin: this list overrides the
    plugins selection generated by merging ``plugins`` and
    ``text_only_plugins``.

``text_removed_plugins``
    A list of plugins which are **not** available in the TextPlugin even if
    they are available in the placeholder; this setting removes **any**
    plugin mentioned in ``plugins``, ``text_only_plugins`` and
    ``text_plugin``.
